      ...
      {items.map((item, idx) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative px-4 py-2",
              isActive ? "text-white" : hovered === idx ? "text-white" : "text-neutral-300"
            )}
            key={`link-${idx}`}
            // href={item.link}
          >
            {(hovered === idx || isActive) && (
              <motion.div
                layoutId="hovered"
                className="absolute inset-0 h-full w-full rounded-full bg-purple-500"
              />
            )}
...
export const NavbarLogo = () => {
  return (
    ...
    >
      <span className="font-medium text-white logo-font text-3xl">Mikun</span>
    </a>
  );
};
